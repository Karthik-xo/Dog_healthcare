...
const LANGS = [
    { key: 'English', code: 'gb', short: 'EN' },
    { key: 'Tamil',   code: 'in', short: 'IN' },
    { key: 'Russian', code: 'ru', short: 'RU' },
];
            ...
            {isMini && (
              <div className="flex items-center justify-between px-3 py-2.5 bg-slate-800 border-b border-white/10 shrink-0 select-none">
                <div className="flex items-center gap-2">
                  ...
                  {subLang && (
                    <span className="flex items-center gap-1.5 bg-blue-600 text-white px-1.5 py-0.5 rounded-full font-black uppercase text-[9px]">
                      <img src={`https://flagcdn.com/w20/${LANGS.find(l => l.key === subLang)?.code}.png`} alt="" className="w-3 h-2.5 object-cover rounded-sm" /> CC
                    </span>
                  )}
                </div>
            ...
            <div className={`flex overflow-hidden flex-1 ${isMini ? 'flex-col' : 'flex-col md:flex-row'}`}>
              ...
              <div
                ...
                {subLang && !isMini && (
                  ...
                  >
                    <img src={`https://flagcdn.com/w40/${LANGS.find(l => l.key === subLang)?.code}.png`} alt="" className="w-4 h-3 object-cover rounded-sm" />
                ...
                <div className={`absolute inset-0 bg-gradient-to-t from-black/85 via-transparent to-black/40 transition-opacity duration-300 ${isMini || (isMobile && showControls) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {subLang && (
                          <motion.span initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                            className="flex items-center gap-2 text-[9px] md:text-[10px] font-black text-blue-300 tracking-[0.15em] bg-blue-500/10 backdrop-blur-md px-2 md:px-3 py-1.5 rounded-full border border-blue-400/20">
                            <img src={`https://flagcdn.com/w40/${LANGS.find(l => l.key === subLang)?.code}.png`} alt="" className="w-3.5 h-2.5 md:w-4 md:h-3 object-cover rounded-sm" />
                            CC · {isMobile ? LANGS.find(l => l.key === subLang)?.short : subLang}
                          </motion.span>
                        )}
                      </div>
                                ...
                                {LANGS.map(l => (
                                  ...
                                  >
                                    <span className="flex items-center gap-2.5">
                                      <img src={`https://flagcdn.com/w40/${l.code}.png`} alt="" className="w-5 h-3.5 object-cover rounded-sm shadow-sm" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <div ref={progressBarRef}
                      ...
                      className="relative h-2 md:h-1 bg-white/20 rounded-full mb-3 cursor-pointer group/progress"
                    >
              ...
              {!isMini && (
                <div className="w-full md:w-80 bg-white flex flex-col overflow-y-auto shrink-0 flex-1 md:flex-none">
                  <div className={`${isMobile ? 'p-5' : 'p-6'} flex-1`}>
                    <div className={`flex items-center justify-between ${isMobile ? 'mb-4' : 'mb-7'}`}>
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-slate-900 rounded-xl md:rounded-2xl"><Sparkles className="text-yellow-400" size={isMobile ? 14 : 17} /></div>
                        <h4 className="font-black text-slate-900 text-sm md:text-base uppercase tracking-tight">Quick Care Tips</h4>
                      </div>
                      <motion.button
                      ...
                      {tips.map((tip, i) => (
                        <motion.div key={i} initial={{ x: 16, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.07 * i }} className="group cursor-default">
                          <div className="flex items-start gap-3 md:gap-4">
                            <div className="mt-0.5 transform group-hover:scale-125 group-hover:rotate-6 transition duration-500 shrink-0">
                              {tip.icon}
                            </div>
                            <div>
                              <h5 className="font-black text-slate-900 text-[11px] md:text-xs uppercase tracking-tight mb-0.5 group-hover:text-blue-600 transition">{tip.title}</h5>
                              <p className="text-slate-500 text-[10px] md:text-[11px] leading-relaxed md:leading-5">{tip.desc}</p>
                            </div>
                          </div>
                        </motion.div>
                      ))}
                    ...
                    {/* ── Subtitle Language Picker (Premium Design) ── */}
                    <div className="mt-8 bg-white rounded-[2.5rem] p-6 shadow-[0_30px_60px_-12px_rgba(0,0,0,0.08)] border border-slate-100 relative overflow-hidden group/card">
                      {/* Subtle background glow */}
                      <div className="absolute -top-24 -right-24 w-48 h-48 bg-blue-50 rounded-full blur-3xl opacity-50 group-hover/card:opacity-80 transition-opacity"></div>
                      ...
                      <div className="flex items-center justify-between mb-8 relative z-10">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em]">Subtitles / CC</span>
                        </div>
                        <button 
                          onClick={clearSub} 
                          className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all px-3 py-1 rounded-full ${!subLang ? 'bg-blue-50 text-blue-600' : 'text-slate-300 hover:text-slate-500'}`}
                        >
                          Off
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-3 relative z-10">
                        {LANGS.map(l => {
                          const isActive = subLang === l.key;
                          return (
                            <button 
                              key={l.key} 
                              onClick={() => setSubLang(isActive ? null : l.key)}
                              className={`relative group flex flex-col items-center justify-center py-5 px-1 rounded-3xl transition-all duration-500 border ${
                                isActive 
                                  ? 'bg-blue-600 border-blue-500 text-white shadow-2xl shadow-blue-200 scale-105 z-10' 
                                  : 'bg-white border-slate-100 text-slate-400 hover:border-blue-200 hover:bg-slate-50/80 hover:scale-[1.02]'
                              }`}
                            >
                              <div className={`mb-3 transition-all duration-500 ${isActive ? 'scale-110 rotate-0' : 'group-hover:scale-110 group-hover:-rotate-3'}`}>
                                 <img 
                                   src={`https://flagcdn.com/w80/${l.code}.png`} 
                                   alt={l.key} 
                                   className={`w-10 h-7 md:w-12 md:h-8 object-cover rounded-lg shadow-md border-2 ${isActive ? 'border-white/50' : 'border-white shadow-sm'}`} 
                                 />
                              </div>
                              <span className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-white' : 'text-slate-500'}`}>
                                {l.key}
                              </span>
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-center gap-3 relative z-10">
                        <div className={`w-2 h-2 rounded-full transition-all duration-500 ${subLang ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]' : 'bg-slate-200'}`} />
                        <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-500 ${subLang ? 'text-blue-600' : 'text-slate-400'}`}>
                          {subLang ? `${subLang} subtitles active` : 'Select a language to sync'}
                        </span>
                      </div>
                    </div>
